...
export default class PointListComponent implements OnInit {
  ...
  constructor(
    ...
    private readonly pointListService: PointListService
  ) {}
  ...
  loadPoints(): void {
    ...
    this.pointListService
      .getPoints(this.offset, this.limit, this.searchBy, this.searchQuery)
      .subscribe(
        (response: any) => {
        ...
        (error) => {
          ...
          this.errorMessage = '포인트 목록 불러오기 실패: ' + error.message;
          console.error('포인트 목록 불러오기 실패:', error);
        }
      );
  }
  ...
  goToDetail(pointId: number): void {
    this.router.navigate([`/points`, pointId]);
  }

  goToAddPoint(): void {
    this.router.navigate(['/points/add']);
  }
